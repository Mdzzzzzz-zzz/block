import { _decorator, Node, ProgressBar, UITransform, instantiate, tween, Animation, UIOpacity, Vec3, Prefab } from 'cc';

import { AdventureLevelConditionViewNew } from './AdventureLevelConditionViewNew';
...
@ccclass("AdventureLevelConditionViewSceneNew")
export class AdventureLevelConditionViewSceneNew extends AdventureLevelConditionViewNew {
    @property(Node)
    barScore: Node = null;
    @property(Node)
    ...
    initConditions() {
        ...
        if (this.manager.targetScore > 999) {
            this.labTargetScore.fontSize = 25;
        } else {
            this.labTargetScore.fontSize = 30;
        }
    }
    ...
    doDiamondsAnim() {
        ...
        for (let i = 0; i < this.diamondsToFly.length; i++) {
            ...
            if (value.key === 506) {
                ResManager.getInstance().loadAsset<Prefab>("res/prefab/blocks/duck/ef_duck_fly_pb", "block").then((prefab) => {
                    let cloneItem = instantiate(prefab);
                    console.log("cloneItemLayer: ", cloneItem.layer);
                    cloneItem.setParent(item.node.parent.parent);
                    cloneItem.setPosition(pos.x, pos.y);
                    cloneItem.active = false;
                    this.scheduleOnce(() => {
                        cloneItem.active = true;
                        let diamondItem = cloneItem.getComponent(Animation);
                        diamondItem.play();
                        this.scheduleOnce(() => {
                            // dimaondItem.playFlyDiamondFlyAnim(); 不用旋转
