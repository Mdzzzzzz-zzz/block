 ...
 * @LastEditTime: 2024-05-08 19:38:03
 */
...
@ccclass('BlockPlaceholderHRocket')
export class BlockPlaceholderHRocket extends BlockPlaceholder3 {
    @property(Node)
    leftRocket: Node = null;
    @property(Node)
    rightRocket: Node = null;
    ...
    public moveToPosition(position: Vec3, callback, lastChooseNodeIndex: number[][]) {
        ...
        tween(this.node)
            .by(this.moveTime1, { position: position }).then(tween(this.node).call(() => {
                //播放行消除特效
                this.scheduleOnce(()=>{
                    BoardView.instance.showClearAniWith(v3(0,-this.rocketPos[0][0]* 80 + 280,0),0,1)
                },0.1)
                //格子挨个消除
                this.rocketPos && this.rocketPos.forEach((value) => {
                    for(let i = 0;i<8;i++){
                        ...
                        },Math.abs(i-value[1])*(1/8)*1000)
                    }
                });

                this.leftRocket.active=true;
                this.rightRocket.active=true;
                let left = tween(this.leftRocket).by(1,{position:this.leftRocket.position.clone().add(v3(-600,0,0))},{easing:"linear"});
                let right = tween(this.rightRocket).by(1,{position:this.rightRocket.position.clone().add(v3(600,0,0))},{easing:"linear"});
                right.start();
                left.then(tween(this.node).call(()=>{
                    this.useRocket();
                    this.leftRocket.setPosition(v3(0,0,0));
                    this.rightRocket.setPosition(v3(0,0,0));
                    this.leftRocket.active=false;
                    this.rightRocket.active=false;
                })).start();
            }))
            .start();
    }
