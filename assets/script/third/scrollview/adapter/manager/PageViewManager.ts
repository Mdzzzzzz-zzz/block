...
export class PageViewManager extends Manager<Event> {
    ...
    private _register() {
        ...
        this.adapter.viewManager.on(VMEvent.ON_UPDATE_VIEWS, this._onUpdateViews, this, true);
    }
    private _unregister() {
        if (!this.adapter || this.enabled) return;
        ...
        this.adapter.viewManager.off(VMEvent.ON_UPDATE_VIEWS, this._onUpdateViews, this);
    }
    public scrollToPage(duration: number, index: number, alwaysScroll?: AlwaysScroll) {
        ...
        this.emit(Event.ON_SCROLL_PAGE_BEFOR, index, dir);
    }
    /**
     * 滚动到上一页
     * @param duration 持续时间
    ...
    public scrollToPrevPage(
        duration: number = this.pageTurningSpeed,
        alwaysScroll: AlwaysScroll = AlwaysScroll.Header
    ) {
    ...
    public scrollToNextPage(
        duration: number = this.pageTurningSpeed,
        alwaysScroll: AlwaysScroll = AlwaysScroll.Footer
    ) {
